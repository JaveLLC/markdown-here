...
/*
 * This file is the heart of Sitthefuckdown Here. It decides whether we're rendering
 * or reverting; whether we're doing a selection or the whole thing; and
 * actually does it (calling out for the final render).
 */

/*
Regarding our rendered Sitthefuckdown "wrappers": When we render a some Sitthefuckdown
-- whether it's the entire content or sub-selection -- we need to save the
original Sitthefuckdown (actually, Sitthefuckdown-in-HTML) somewhere so that we can later
unrender if the user requests it. Where to save the original sitthefuckdown is a
...
function getOperationalRange(focusedElem) {
  ...
  if (sig) {
    ...
    if (sig.nodeType === sig.ELEMENT_NODE) {
      sig.classList.add('sitthefuckdown-here-signature');
    }
...
// NOTE: I would really prefer that this be in sitthefuckdown-render.js with the other
...
function replaceRange(range, html) {
  ...
  // the newly inserted rendered Sitthefuckdown will be selected. It looks better and
...
function getSitthefuckdownStylesheet(elem, css) {
  ...
  styleElem.setAttribute('title', 'sitthefuckdown-here-styles');
  ...
  for (i = 0; i < elem.ownerDocument.styleSheets.length; i++) {
    if (elem.ownerDocument.styleSheets[i].title === 'sitthefuckdown-here-styles') {
  ...
  if (!stylesheet) {
    throw 'Sitthefuckdown Here stylesheet not found!';
  }
...
function makeStylesExplicit(wrapperElem, css) {
  ...
  stylesheet = getSitthefuckdownStylesheet(wrapperElem, css);
  ...
  for (i = 0; i < stylesheet.cssRules.length; i++) {
    ...
    for (j = 0; j < selectorMatches.length; j++) {
      ...
      while (elem && (typeof(elem.classList) !== 'undefined')) {
        if (elem.classList.contains('sitthefuckdown-here-exclude')) {
          elem = 'excluded';
          break;
        }
...
function findElemRawHolder(elem) {
  ...
  for (var i = 0; i < rawHolders.length; i++) {
    if (// The above `querySelector` will also look at grandchildren of
        ...
        // Sitthefuckdown that was sent to us.
...
function findSitthefuckdownHereWrapper(focusedElem) {
...
// Finds all Sitthefuckdown Here wrappers in the given range. Returns an array of the
// wrapper elements, or null if no wrappers found.
function findSitthefuckdownHereWrappersInRange(range) {
...
// Converts the Sitthefuckdown in the user's compose element to HTML and replaces it.
// If `selectedRange` is null, then the entire email is being rendered.
function renderSitthefuckdown(focusedElem, selectedRange, sitthefuckdownRenderer, renderComplete) {
  ...
  sitthefuckdownRenderer(focusedElem, selectedRange, function(mdHtml, mdCss) {
    var wrapper, rawHolder;

    // Store the original Sitthefuckdown-in-HTML to the `title` attribute of a separate,
    ...
    mdHtml =
      '<div class="sitthefuckdown-here-wrapper" ' +
    ...
    wrapper.ownerDocument.defaultView.setTimeout(function addMutationObserver() {
      ...
      if (typeof(SupportedMutationObserver) !== 'undefined') {
        var observer = new SupportedMutationObserver(function(mutations) {
          wrapper.setAttribute('sitthefuckdown-here-wrapper-content-modified', true);
          observer.disconnect();
        });
...
// Revert the rendered Sitthefuckdown wrapperElem back to its original form.
function unrenderSitthefuckdown(wrapperElem) {
...
// @param `sitthefuckdownRenderer`  The function that provides raw-Sitthefuckdown-in-HTML
//                            to pretty-Sitthefuckdown-in-HTML rendering service.
...
//          to the user.
function sitthefuckdownHere(document, sitthefuckdownRenderer, logger, renderComplete) {

  if (logger) {
    mylog = logger;
  }

  // If the cursor (or current selection) is in a Sitthefuckdown Here wrapper, then
  // we're reverting that wrapper back to Sitthefuckdown. If there's a selection that
  ...
  // Look for existing rendered-Sitthefuckdown wrapper to revert.
  outerWrapper = findSitthefuckdownHereWrapper(focusedElem);
  if (outerWrapper) {
    ...
    wrappers = findSitthefuckdownHereWrappersInRange(range);
  }
  ...
  if (wrappers && wrappers.length > 0) {
    ...
    for (i = 0; i < wrappers.length; i++) {
      ...
      if (wrappers[i].getAttribute('sitthefuckdown-here-wrapper-content-modified') &&
          !yesToAll) {

          if (wrappers[i].ownerDocument.defaultView.confirm(Utils.getMessage('unrendering_modified_sitthefuckdown_warning'))) {
            yesToAll = true;
          }
          else {
            break;
      ...
      unrenderSitthefuckdown(wrappers[i]);
    }
...
sitthefuckdownHere.findFocusedElem = findFocusedElem;
sitthefuckdownHere.elementCanBeRendered = elementCanBeRendered;

var EXPORTED_SYMBOLS = ['sitthefuckdownHere'];

if (typeof module !== 'undefined') {
  module.exports = sitthefuckdownHere;
} else {
  this.sitthefuckdownHere = sitthefuckdownHere;
